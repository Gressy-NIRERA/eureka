...
class Api {
 ...
 Future<List<dynamic>> searchDeliveryPlace(String query) async {
  try {
    final response = await dio.post(
...
/// Calcule les prix disponibles (duma basic, duma moto, etc.) pour une
/// distance donnée (en km).
/// Endpoint réel : POST https://taxi.duma.africa/api/v1/price/2
/// Body réel (vérifié via Postman) :
/// {
///   "distance": 5.8,
...
///     {
///       "race": 2,
///       "race_type": { "id": 2, "name": "duma basic", "max_person": "4",
///                       "photo": "...", "visible": 1 },
///       "price": 31900,
///       "price_default": "11000",
///       "night": 0,
...
///   ]
/// }
Future<List<dynamic>> getPrice(double distance, {String country = "BI"}) async {
  try {
    ...
    if (response.data is Map && response.data["prices"] is List) {
      return List<dynamic>.from(response.data["prices"]);
    }
    return [];
  } on DioException catch (e) {
...
Future<List<dynamic>> getCompanies(String isoCode, {int page = 1}) async {
  try {
    final response = await dio.get(
    ...
    if (body is Map && body["data"] is Map && body["data"]["data"] is List) {
      return List<dynamic>.from(body["data"]["data"]);
    }
    if (body is Map && body["companies"] is List) {
      return List<dynamic>.from(body["companies"]);
    }
    ...
    print("COMPANIES : format de réponse inattendu, voir COMPANIES RESPONSE ci-dessus");
    return [];
  } on DioException catch (e) {
    print("Erreur restaurants (Dio) : ${e.response?.statusCode} ${e.response?.data}");
    return [];
  } catch (e) {
...
Future<Map<String, dynamic>?> getCompanyDetail(dynamic id) async {
  try {
    final response = await dio.get(
    ...
    print("COMPANY DETAIL RESPONSE: ${response.data}");
    ...
    print("COMPANY DETAIL : format de réponse inattendu, voir COMPANY DETAIL RESPONSE ci-dessus");
    return null;
  } on DioException catch (e) {
    print("Erreur détail restaurant (Dio) : ${e.response?.statusCode} ${e.response?.data}");
    return null;
  } catch (e) {
